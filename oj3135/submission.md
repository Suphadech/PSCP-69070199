...
```text
โจทย์ให้หาจำนวนคนที่ได้เช็คกล่องของขวัญโดยจะมีโต๊ะทรงกลม พร้อมกับมีคนนั่งรอบโต๊ะ n คนจะเริ่มโดยการให้คนแรกเช็คก่อนหลังจากนั้นก็ส่งไปให้คนที่ 1+k ไปเรื่อยๆเช่น 1,1+k,1+2k,... โดยจะมีการส่งวนไปเรื่อยๆจนกว่า
(1) ของขวัญวนกลับมาที่คนที่ 1
(2) ของขวัญวนมาให้ขโมยพิจารณาตรวจสอบ ขโมยคือคนที่ t เราต้องกำหนดจาก input

Input:
ค่า n แทนจำนวนคนรอบโต๊ะ,ค่า k แทนกับส่งต่อจากคนที่ได้เช็คก่อนหน้า,ค่า t แทนโจรที่จะขโมย

Output:
แทนจำนวนคนที่ได้พิจารณาของขวัญนับรวมขโมยด้วย
```
...
```text
Step1:เขียน Module Docstring กัน PEP-8
Step2:รับค่า Input n,k,t เอาไว้รับค่าตามที่โจทย์อธิบายพร้อมกำหนดตัวแปร current คือตำแหน่งปัจจุบันที่ตรวจสอบและ count เอาไว้นับคนที่ตรวจสอบโดยจะไม่นับคนซ้ำ
Step3:ใช้ if เช็คหากตำแหน่ง current เท่ากับ ตำแหน่งโจรก็จะทำการ print ค่า count เลย
Step4:ส่วน else ก็จะเช็คตามปกติโดยการใช้ While-loop ในการหาไปเรื่อยๆพร้อมคำนวณค่า current  ณ ตอนนั้น
Step5:ใช้ if เช็คหาก current == 1 หรือตำแหน่งคนแรกก็จะใช้ break ในการหลุด loop เลยหากไใ่ใช่ก็จะไม่มีอะไรเกิดขึ้นจากนั้นก็เพิ่ม count เข้าไปหนึ่ง
Step6:ใช้ if เช็คเงื่อนไขอีกครั้งโดยหาก current == t หรือว่าคือตำแหน่งโจรก็จะหลุด loop ทันทีเลย
Step7:ปริ้น Output
```
...
```text
วิธีสุดท้ายของผมเหมือนกับแผนแรก แต่จะมีการปรับเปลี่ยน code ค่อนข้างเยอะเพราะ code แรกนั้นใช้การเขียนที่ซับซ้อนจนเกินไปจนต้องแก้ให้เหลือแต่อันที่จำเป็นก็จะดูเรียบร้อยขึ้น
```
...
```text
Check ตาม sample testcase ของ i judge
```
