...
```text
โจทย์ให้หาการเกิด Surprising เมื่อมีโอกาศที่เลขมากที่สุดห่างกับเลขน้อยที่สุดเกินสองคะแนน

Input:
total คะแนนรวมของทั้งสามคน
max_score คะแนนที่สูงที่สุด

Output:
"Surprising"  เมื่อมีโอกาศที่เลขมากที่สุดห่างกับเลขน้อยที่สุดเกินสองคะแนน
"Not surprising" เมื่อไม่มีโอกาศเลย
```
...
Step 2: นั่งทำความเข้าใจโจทย์
Step 3: รับค่า Input totalและmax_score คือคะแนนรวมของทั้งสามคนและคะแนนสูงสุด
Step 4: สร้างตัวแปร min_score เผื่อหาค่าที่น้อยที่สุดที่เป็นไปได้ด้วยการสมมุติว่าอีกค่าหนึ่งนั้นแย่งคะแนนตัวที่น้อยที่สุดไปหมดจะมีค่าเท่ากับ max_score เพื่อบีบให้ค่าน้อยที่สุดเท่าที่จะเป็นไปได้ด้วยการเอาคะแนนรวมลบด้วย max_score * 2 ก็จะได้ค่า  min_score
Step 5: ใช้ if เช็คเงื่อนไขหากเอา max_score - min_score แล้วมากกว่า 2 ก็ให้ปริ้น Surprising หากไม่ใช่ก็ให้ปริ้น Not surprising
```
...
```text
วิธีสุดท้ายของผทเหมือนกับแผนแรก แต่หลังจากที่เอา code ไปส่งก็ยังติดบาง testcase ที่ไม่ผ่านเลยมาพิจารณาใหม่โดยสรุปได้ว่า ใน Step 4 ของแผนแรกผมต้องทำการครอบด้วย max(0, )เพื่อกันไม่ให้ค่า min_score ติดลบเพราะขอบเขตที่เป็นได้ที่สุดคือ 0 หลังจากแก้ก็ทดสอบหลายๆ testcase
```
...
```text
ทดสอบเพื่อให้ถูกตาม testcase ของ <i> judge
```
...
```text
ทดสอบในกรณีที่ min_score เป็นลบแล้วใช้ max
```
