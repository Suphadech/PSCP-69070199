...
Input: รับ input สี 2 สี

Output: นำเอาสีจาก input มาผสมแล้วแสดงผลว่าได้สีอะไร หาก input เป็นสีอื่นที่นอกจากแม่สีที่กำหนดให้ให้แสดงผล "Error"
```
...
Step 2: รับ Input 2 ค่าแล้วเก็บไว้ในตัวแปร color1,color2 ตามลำดับ แล้วใช้ .capitalize ในการทำให้ตัวอักษรตัวหน้าสุดเป็นตัวพิมพ์ใหญ่กันพลาด
Step 3: ใช้ if-elif ตรวจสอบว่าหาก color 1 เป็น หนึ่งในแม่สี และ color 2 เป็นหนึ่งในแม่สีที่แตกต่างกัน และ ใช้ or เพื่อที่เช็คว่าหาก color1 กับ color2 เป็นสีที่แค่สลับ input แต่ก็ได้ผลลัพธ์เดียวกันเพื่อลด branch ของ if-elif
step 4 : ใช้ else เพื่อที่แยกเอาไว้สำหรับ input ที่ไม่มีในแม่สีให้ print error
Step 5: ดู output ที่ออกมา
```
...
```text
Step 1: เขียน Module docstring กัน PEP-8
Step 2: รับ Input 2 ค่าแล้วเก็บไว้ในตัวแปร color1,color2 ตามลำดับ แล้วใช้ .capitalize ในการทำให้ตัวอักษรตัวหน้าสุดเป็นตัวพิมพ์ใหญ่(กันพลาด)
Step 3: ใช้ if-elif ตรวจสอบว่าหาก color 1 เป็น หนึ่งในแม่สี และ color 2 เป็นหนึ่งในแม่สีที่แตกต่างกัน และ ใช้ or เพื่อที่เช็คว่าหาก color1 กับ color2 เป็นสีที่แค่สลับ input แต่ก็ได้ผลลัพธ์เดียวกันเพื่อลด branch ของ if-elif
step 4 : เนื่องจากแผนแรกผมลืมกรณีที่หากเอาสีเดียวกันมาผสมกันเลยใช้ elif หาก color1 == สีของแม่สีนั้นๆและcolor2 ก็ == สีของแม่สีนั้นๆที่เหมือนกันก็จะแสดง output เป็นสีของแม่สีนั้นๆ
step 5 : ใช้ else เพื่อที่แยกเอาไว้สำหรับ input ที่ไม่มีในแม่สีให้ print error
Step 5: ดู output ที่ออกมา
```
...
```text
ทดสอบว่าจะถูกตามเงื่อนไข if-elif ไหม
```
